...
# Display order for blog category filters (only categories with posts are shown).
BLOG_CATEGORY_ORDER = [
...
def get_blog_categories(posts):
    """Return categories that have at least one post, in BLOG_CATEGORY_ORDER."""
    present = {post.category for post in posts if post.published}
    ordered = [category for category in BLOG_CATEGORY_ORDER if category in present]
